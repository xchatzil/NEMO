...
void* APEXCALL tiberium(void* dst,
                        const void* src,
                        size_t size) {// based on memmove09 for "size <= 112" and memmove40 for "size > 112"
    if (size <= 112) {
        if (size >= 16) {
...
void* APEXCALL kryptonite(
    void* dst,
    const void* src,
    size_t
        size) {// based on memmove09 for "size <= 112" and memmove41 for "size > 112"; memmove09's "size <= 112" proved fastest overall (weighted), even on Core i5!
    if (size <= 112) {
        if (size >= 16) {
            const __m128i xmm0 = _mm_loadu_si128((__m128i*) src);
            if (size > 16) {
...
#define bit_SSE4_2 (1 << 20)
    ...
    if (cpuid[2]
        & bit_SSE4_2)//	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
